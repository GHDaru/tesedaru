% ============================================================================
% ESQUEMA PROPOSTO (banca, sugestão — não mergeado): a geometria da LCE e o
% mapa da análise estatística (Seção sec:metodo-metricas). A curva é
% ILUSTRATIVA e o desenho diz isso no título interno: nenhum dado medido,
% nenhum número além dos símbolos da própria equação — é a definição da
% métrica desenhada, não um resultado (compatível com o princípio V).
%
% Uso no Cap. 3 (dentro de um ambiente figure):
%   \input{3-metodo/esquemas-propostos/esq-lce-e-estatistica}
%   As citações (Guyon2011ALC, Wilson1927, McNemar1947, Wilcoxon1945)
%   permanecem na prosa/legenda — a figura usa apenas os nomes dos métodos.
% Uso standalone (pré-visualização):
%   \documentclass[tikz,border=6pt]{standalone}
%   \usetikzlibrary{arrows.meta, positioning}
%   \begin{document}\input{esq-lce-e-estatistica.tex}\end{document}
...
\begin{tikzpicture}[
    ...
    \node[caixa, text width=52mm] (formula) at (10.4,3.7)
    ...
    \node[rot, text width=50mm, anchor=north, align=left] at (10.4,2.5)
    ...
    \node[rot, text width=50mm, anchor=north, align=left] at (10.4,0.5)
        {precedente: a ALC do \textit{Active}\\
         \textit{Learning Challenge}. Difere em dois\\
         pontos: normaliza pelo teto do próprio\\
         par (classificador, conjunto), comparável\\
         entre tetos distintos; e integra por\\
         Simpson na grade observada, sem\\
         interpolação em escala logarítmica.};

    % ---------------- faixa inferior: análise estatística ------------------
    \node[rot, anchor=west] at (-0.4,-2.4)
        {\textbf{Análise estatística} --- o teste segue a natureza dos dados
         (referências na prosa da Seção~\ref{sec:metodo-metricas}):};

    \node[caixa, text width=40mm] at (1.6,-3.8)
        {\textbf{IC de Wilson a 95\%}:\\toda acurácia de oráculo\\
         (proporção em qualquer faixa)};
    \node[caixa] at (6.4,-3.8)
        {\textbf{teste de McNemar}: oráculos\\sobre as mesmas instâncias\\
         (pares discordantes; binomial\\exato se $<25$ discordâncias)};
    \node[caixa] at (11.2,-3.8)
        {\textbf{Wilcoxon} (postos sinalizados):\\
         estratégias com múltiplas\\sementes, sobre as métricas\\
         finais e a LCE};
\end{tikzpicture}
