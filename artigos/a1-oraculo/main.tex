...
\begin{abstract}
Large language models (LLMs) are increasingly used as labeling oracles, but
the evidence base concentrates on tasks with tens to a few hundred classes
and rarely treats cost or the measurement instrument as objects of study.
We evaluate six LLM oracles on zero-shot classification of short Portuguese
retail product descriptions into a closed space of 621 categories --- well
beyond prior product-classification studies --- under a fully instrumented
protocol: output constrained to the label space, batch annotation with
paired calibration, cache-aware cost accounting, serving provenance, and
paired statistics (Wilson intervals, exact McNemar). Four oracles plateau
at 77--83\% accuracy, $\approx 7$ points under a supervised ceiling trained
on 250{,}000 labels, at US\$~0.00--0.92 per thousand labels; the
best-value model delivers 95\% of the top accuracy at 8.5\% of its cost,
and a zero-cost oracle is statistically tied with it. Plateau Macro-F1
\emph{exceeds} a fully supervised light baseline, because LLMs are
strongest precisely in rare classes. Nearly half of oracle error reflects
catalog conventions rather than product misunderstanding. Two findings
concern the instrument itself: without output constraints, 6.8\% of
responses become spurious phrasing errors charged against the model; and
the same model served by two providers disagrees with itself ($p<0.001$)
on identical instances. A prompt ablation with boundary rules improves the
production distribution ($+4.6$ points, $p=0.012$) while severely
degrading the stratified tail ($-10.8$ points, $p<0.001$). Code, prompts
and per-instance annotation artifacts are released.
\end{abstract}
...
noisy-annotator tradition \citep{Snow2008,Sheng2008}; a known caveat is
that LLM self-reported confidence tends toward overconfidence and requires
calibration \citep{Tian2023}, which is why our protocol relies on external
measurement rather than model-reported certainty. Evaluations at
...
\paragraph{Instrument-mode assignment and scoring convention.} The output
contract is dictated by provider capability: enum for providers with
structured-output support (OpenAI), json-prompt elsewhere. This is a
potential confound between provider families, which the protocol handles by
(a) reporting invalid-label rates separately in every table, (b) scoring
invalid outputs as errors --- so json-prompt oracles are penalized, not
shielded, by the weaker contract --- and (c) quantifying the maximal size of
the contract effect directly in RQ4 with the same model under both
contracts. Cost is computed from provider-metered token counts per call
multiplied by the provider price sheet at execution date, with cached
prefix tokens billed at the provider's cached rate; all per-call token
counts are in the released artifacts.
...
throughout, which reduces but does not eliminate serving nondeterminism ---
each cell is a single annotation run, a limitation stated in
Section~\ref{sec:discussion}. Accuracies carry Wilson 95\% intervals
...
evaluated once on a common sample. Two comparisons were pre-registered as
decision-relevant (best vs.\ runner-up for the advanced-oracle role;
zero-cost vs.\ cheapest-paid for the initial-oracle role); remaining
pairwise tests are reported as exploratory, and no result in this paper
depends on a marginal $p$-value surviving multiplicity correction.
...
Errors of the best oracle (v4-pro, S-strat; $n=324$) were categorized by
scripted lexical rules (shared head-word between predicted and gold label;
umbrella-class markers; sentinel/invalid detection) with manual
spot-checking; the categorization script and the labeled error list are in
the released artifacts. The result: \textbf{31\%}
...
sample --- a prefix subset (random by construction, as samples are
pre-shuffled) sized to detect shifts of 4--5 points via McNemar given the
expected 60--80 discordant pairs, at one fifth of the full-sample cost per
variant. On S-rand, rules recover part of the convention error: $+3.8$
...
\citep{Raczkowska2024AlleNoise} as future work); a single annotation run
per cell --- temperature 0 reduces but does not eliminate serving
nondeterminism, and the RQ2 provider-divergence finding quantifies exactly
why this matters; API prices dated at execution (ratios are more stable
than absolute values); boundary rules in the ablation derive from this
catalog's error profile (the method generalizes; the rules do not);
gold-standard noise bounds all accuracies at $\approx 99.3\%$, audited and
propagated.
...
\paragraph{Artifacts.} Code, prompts, per-instance annotations, token
counts and analysis scripts: \texttt{github.com/GHDaru/activelearning}
(\texttt{experiments/e0}, \texttt{e0p}). The underlying corpus is
publicly available on Kaggle (DOI \texttt{10.34740/kaggle/dsv/4265348});
the paired evaluation samples and all annotation artifacts are released
with this paper. \todo{DOI/Zenodo release do código antes da submissão}
