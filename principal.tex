% Versão 24/06/2020
% Rebuild do PDF: 2026-08-22 (main atual, para leitura do autor)
...
% título
\title{Aprendizado Ativo com Modelos de Linguagem \\ para Textos Curtos em Português}

% palavras-chave e keywords (p/ resumo, abstract e metadados do PDF)
\pchave{Aprendizado Ativo. Classificação de Textos Curtos. Modelos de Linguagem de Grande Porte. FALCO. Rotulagem.}
\keyword{Active Learning. Short Text Classification. Large Language Models. FALCO. Labeling.}

% autoria
\author{Gilsiley Henrique Darú}
\advisor{Prof. Dr. Gustavo Valentim Loch}
% \coadvisor{}
...
% área de concentração (PPGMNE)
\IfLanguageName{brazilian}
  { \field{Programação Matemática} }
% a Bib/UFPR exige que tudo seja em português, exceto o título :-(
...
% tese de doutorado
\descr{Tese apresentada como requisito parcial à obtenção do grau de Doutor em Métodos Numéricos em Engenharia no Programa de Pós-Graduação em Métodos Numéricos em Engenharia, Setores de Ciências Exatas e de Tecnologia, da Universidade Federal do Paraná}
...
\include{0-iniciais/declaracao-ia}
...
\include{2-fundam/texto}		% fundamentação teórica e revisão
\include{3-metodo/texto}		% metodologia (FALCO, dados, métricas, desenho experimental)
\include{4-resultados-l0/texto}		% resultados P1/P2: sensibilidade do L0, AG, DRI-SL
\include{5-resultados-falco/texto}	% resultados P3/P4: oráculo LLM, FALCO vs RS/US
\include{6-conclusao/texto}		% discussão e conclusão
...
\include{a1-lce/texto}			% métrica LCE (formalização)
\include{a2-ag/texto}			% algoritmo genético para otimização de L0
\include{a3-drisl/texto}		% algoritmo DRI-SL (cold start)
\include{a4-biblioteca/texto}		% biblioteca activelearning (arquitetura e reprodução)
\include{a5-prompts/texto}		% prompts e schemas do oráculo LLM
\include{a6-tabelas/texto}		% tabelas completas (estatísticas de L0)
\include{a7-parada-drift/texto}	% parada, liberação e monitoramento de deriva
