...
para: principal
tipo: tarefa
acao_esperada: despachar ao revisor1 a tarefa das duas figuras do Cap. 3 — o texto abaixo, entre as linhas de corte, está pronto para você copiar e colar como mensagem sua ao revisor1, sem edição; registrar também a exceção de roteamento que o autor autorizou (revisor1 fala direto com o autor NESTA tarefa, para mostrar a figura antes da aprovação)
referencia: ordem do autor à banca, 2026-08-18 ("Passe para o revisor 01 a tarefa das figuras, peça para ele interagir comigo mostrando a figura antes de eu aprovar... ele deve trabalhar em loop e ele mesmo dar uma nota até ficar convencido") · achado das figuras no §3 da minha entrega 0035 · ADR 0009 (roteamento pelo principal)
criada_em: 2026-08-18T00:55:00Z
---
Duas observações antes do bloco:

1. EXCEÇÃO DE ROTEAMENTO, autorizada nominalmente pelo autor: nesta tarefa o
   revisor1 interage DIRETO com ele para mostrar a figura e colher a
   aprovação. Não é revogação do ADR 0009; vale para esta tarefa e para o
   ato de mostrar/aprovar figura. Planejamento e gate de merge continuam
   seus.
2. O autor pediu explicitamente o modo de trabalho: LOOP com AUTOAVALIAÇÃO.
   O revisor1 se dá uma nota, itera, e só para quando ele próprio estiver
   convencido de que a nota está boa. Deixei a rubrica escrita para que a
   nota não seja opinião solta.
