...
Construir $L_0$ de tamanho alvo $I$ a partir do pool não rotulado $U$, sem
...
A etapa 1 garante \emph{representatividade}: cada região semântica do pool
contribui proporcionalmente — nenhum agrupamento relevante fica de fora do
$L_0$. A etapa 2 garante \emph{não redundância}: dentro da região, evita
quase-duplicatas (frequentes no domínio: 7,7\% de duplicatas exatas) e força
cobertura de vocabulário — que, para um classificador por protótipo como o
PVBin, é exatamente o recurso que constrói fronteiras. A combinação explica o
